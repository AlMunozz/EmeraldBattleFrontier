...
$(document).ready(function () {
	...
	calculate();
	gen = 3;                          // variable global de generación
    $("#gen3").prop("checked", true).change();

    // Opcional: ocultar visualmente otras gens
	$("#gen3, label[for='gen3']").hide();
	$("#gen4, label[for='gen4']").hide();
    $("#gen5, label[for='gen5']").hide();
    $("#gen6, label[for='gen6']").hide();
    $("#gen7, label[for='gen7']").hide();

	$("#one-vs-one").prop("checked", true).change();

    // Ocultar TODOS los botones de modo
    $("#one-vs-one, label[for='one-vs-one']").hide();
    $("#one-vs-all, label[for='one-vs-all']").hide();
    $("#all-vs-one, label[for='all-vs-one']").hide();

	$("#singles-format").prop("checked", true);
    $("#doubles-format").prop("checked", false);
    $("#singles-format").trigger("change");
});
